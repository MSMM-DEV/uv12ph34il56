...
export async function getProjects(category?: string): Promise<Project[]> {
  const filter = category
    ? `*[_type == "project" && "${category}" in category && isHidden != true] | order(coalesce(orderRank, "zzz") asc, _updatedAt desc)`
    : `*[_type == "project" && isHidden != true] | order(coalesce(orderRank, "zzz") asc, _updatedAt desc)`;
...
export async function getFeaturedProjects(): Promise<Project[]> {
  return client.fetch(
    `*[_type == "project" && featured == true && isHidden != true] | order(coalesce(orderRank, "zzz") asc, _updatedAt desc) [0...6] {
      "id": _id,
      name,
      "slug": slug.current,
      category,
